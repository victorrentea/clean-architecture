...
public class ArchitectureTest {
    ...
    public void domain_independent_of_infrastructure() {
        ...
        ClassesShouldConjunction rule = noClasses().that().resideInAPackage("..domain..")
            .should().dependOnClassesThat().resideInAPackage("..infra..");
        rule.check(allProjectClasses);
        List<String> details = rule.evaluate(allProjectClasses).getFailureReport().getDetails();
        assertThat(details).hasSizeLessThanOrEqualTo(4);
    }
