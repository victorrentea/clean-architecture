...
public class IndependentSubdomainsArchUnitTest {
   ...
   public void independentSubdomains() {
      ...
      SliceRule sliceRule = SlicesRuleDefinition.slices()
          .matching("victor.training.clean.(*).*")
          // example: service.order should not depend on service.customer
          .should().notDependOnEachOther()
          .ignoreDependency(DescribedPredicate.alwaysTrue(), Predicates.resideInAnyPackage("..common..", "..infra"))
          ;
      // allow dependencies to .events
      ...
      System.out.println(evaluationResult.getFailureReport());
      System.out.println("Number of violations: " + violations);

      assertThat(violations).isLessThanOrEqualTo(0);
